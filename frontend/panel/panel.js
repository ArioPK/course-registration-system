    ...
    const AdminCourseApp = {
      ...
      async loadInitialData() {
        try {
          ...
          const errorMessage =
            error.message || "خطا در بارگذاری اطلاعات اولیه.";
          alert(errorMessage);
      ...
      populateFilters() {
        ...
        while (this.elements.filterDepartment.children.length > 1) {
          this.elements.filterDepartment.removeChild(
            this.elements.filterDepartment.lastChild
          );
        }
        while (this.elements.filterSemester.children.length > 1) {
          this.elements.filterSemester.removeChild(
            this.elements.filterSemester.lastChild
          );
        }
      ...
      applyState() {
        ...
        this.elements.navLinks.forEach((link) =>
          link.classList.toggle(
            "active",
            link.dataset.target === activeSectionId
          )
        );
      ...
      courses() {
        ...
        filteredCourses.forEach((course) => {
          ...
          const capacityPercent = (enrolled / course.capacity) * 100;
          ...
          tbody.innerHTML += `
              <tr class="${isFull ? "full-row" : ""}">
                ...
                <td style="font-size: 0.85rem; direction: ltr; text-align: right;">
                    ${translateDay(course.day_of_week)}<br>
      ...
      getFilteredCourses() {
        ...
        if (this.state.filterDepartment) {
          filtered = filtered.filter(
            (course) => course.department === this.state.filterDepartment
          );
        }
        ...
        if (this.state.filterSemester) {
          filtered = filtered.filter(
            (course) => course.semester === this.state.filterSemester
          );
        }
      ...
      updateSummary() {
        ...
        const totalCapacity = this.data.courses.reduce(
          (sum, c) => sum + c.capacity,
          0
        );
        const totalEnrolled = this.data.courses.reduce(
      ...
      bindEvents() {
        ...
        // Add course button
        this.elements.addCourseBtn.addEventListener(
          "click",
          this.handlers.onAddCourseClick
        );

        // Course form submission
        this.elements.courseForm.addEventListener(
          "submit",
          this.handlers.onCourseFormSubmit
        );
      ...
      handlers: {
        ...
        onAddCourseClick() {
          ...
          AdminCourseApp.elements.courseModalTitle.textContent =
            "افزودن درس جدید";
        ...
        onEditCourse(courseId) {
          const course = AdminCourseApp.data.courses.find(
            (c) => c.id === courseId
          );
          if (!course) return;
          ...
          AdminCourseApp.elements.courseProfessor.value =
            course.professor_name || "";
          AdminCourseApp.elements.courseDay.value = course.day_of_week || "";
          AdminCourseApp.elements.courseStart.value = course.start_time || "";
          AdminCourseApp.elements.courseEnd.value = course.end_time || "";
          AdminCourseApp.elements.courseLocation.value = course.location || "";
        ...
        async onCourseFormSubmit(e) {
          ...
          const formData = {
            ...
            professor_name:
              AdminCourseApp.elements.courseProfessor.value.trim(),
            day_of_week: AdminCourseApp.elements.courseDay.value,
            start_time: AdminCourseApp.elements.courseStart.value,
            end_time: AdminCourseApp.elements.courseEnd.value,
            location: AdminCourseApp.elements.courseLocation.value.trim(),
          ...
          const submitButton = AdminCourseApp.elements.courseForm.querySelector(
            'button[type="submit"]'
          );
          ...
          try {
            ...
            if (AdminCourseApp.state.currentEditId) {
              ...
              console.log(
                "Updating course:",
                AdminCourseApp.state.currentEditId,
                formData
              );
              result = await AdminCourseApp.api.updateCourse(
                AdminCourseApp.state.currentEditId,
                formData
              );
            } else {
      ...
      ui: {
        ...
        validateForm(formData) {
          ...
          if (!formData.code) {
            ...
            if (
              AdminCourseApp.data.courses &&
              Array.isArray(AdminCourseApp.data.courses)
            ) {
        ...
        showConfirmation(message, onConfirm) {
          ...
          confirmModal.querySelector("#confirmation-message").textContent =
            message;
      ...
      api: {
        ...
        async _request(url, options = {}) {
          ...
          const timeoutId = setTimeout(
            () => controller.abort(),
            AdminCourseApp.API_CONFIG.TIMEOUT
          );
      ...
      enrolled: 35 
        ...
        async addCourse(courseData) {
          if (AdminCourseApp.API_CONFIG.USE_MOCK) {
            ...
            const newId =
              Math.max(0, ...this._mockDB.courses.map((c) => c.id)) + 1;
            const newCourse = {
              id: newId,
              ...courseData,
              enrolled: 0,
        ...
        async updateCourse(id, courseData) {
          ...
          const url = `${
            AdminCourseApp.API_CONFIG.BASE_URL
          }${AdminCourseApp.API_CONFIG.ENDPOINTS.COURSE_BY_ID(id)}`;
        ...
        async deleteCourse(id) {
          if (AdminCourseApp.API_CONFIG.USE_MOCK) {
            ...
            await this._delay(300);
            this._mockDB.courses = this._mockDB.courses.filter(
              (c) => c.id !== id
            );
            return { success: true };
          }

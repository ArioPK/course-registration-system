...
export class CourseController {
    ...
    async loadInitialData() {
      try {
        ...
        const departments = [...new Set(this.state.courses.map((c) => c.department).filter(Boolean))];
        const semesters = [...new Set(this.state.courses.map((c) => c.semester).filter(Boolean))].sort();
    ...
    _getFilteredCourses() {
      ...
      if (this.state.filterDepartment) {
        filtered = filtered.filter((c) => c.department === this.state.filterDepartment);
      }
  
      // Filter by Semester
      if (this.state.filterSemester) {
        filtered = filtered.filter((c) => c.semester === this.state.filterSemester);
      }
    ...
    _updateSummary() {
      ...
      const totalCapacity = this.state.courses.reduce((sum, c) => sum + (c.capacity || 0), 0);
      const totalEnrolled = this.state.courses.reduce((sum, c) => sum + (c.enrolled || 0), 0);
    ...
    _bindEvents() {
      ...
      // Form Submission
      this.view.bindCourseFormSubmit((formData) => this._handleFormSubmit(formData));

...
export class StudentCourseView {
    ...
    renderCourses(courses, prerequisites = []) {
      ...
      courses.forEach((course) => {
        const coursePrereqs = prerequisites.filter(p => p.target_course_id === course.id);
        ...
        card.innerHTML = `
          <div class="course-header">
             ...
             ${hasPrereq ? '<i class="ri-links-line" title="دارای پیش‌نیاز" style="color: var(--accent);"></i>' : ''}
          </div>
          ...
          <div class="course-info">
              ...
              <div class="info-row">
                  ...
                  <span>${this._translateDay(course.day_of_week)} ${course.start_time.slice(0,5)} - ${course.end_time.slice(0,5)}</span>
              </div>
               <div class="info-row">
                  <i class="ri-map-pin-line"></i>
                  <span>${course.location}</span>
              </div>
               <div class="info-row" style="margin-top: 10px; color: ${course.enrolled >= course.capacity ? 'var(--danger)' : '#4caf50'}">
                  <i class="ri-group-line"></i>
                  <span>ظرفیت: ${course.enrolled || 0} / ${course.capacity}</span>
              </div>
          </div>
        `;
